...
class StepIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _Step(number: 1, label: 'DELIVERY',
            done: currentStep > 1, isActive: currentStep == 1),
        _StepLine(active: currentStep > 1),
        _Step(number: 2, label: 'ADDRESS',
            done: currentStep > 2, isActive: currentStep == 2),
        _StepLine(active: currentStep > 2),
        _Step(number: 3, label: 'PAYMENT',
            done: currentStep > 3, isActive: currentStep == 3),
      ],
    );
  }
}
